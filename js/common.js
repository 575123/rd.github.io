...
function returnTVHostIfIs(pname, tv) {
    if (pname.indexOf(tv.toLowerCase()) == 0) return "d," + tv;
    else return "";
}
...
function getPlayerName(pname) {
    if (pname == undefined || pname == "" || !isShowPlayerName) return "";
    else {
        pname = pname.toLowerCase();
        if (isShowAllPlayerName) {
            if (isTVHoster(pname)) pname = "d," + pname;
            return pname;
        }
...
function onResize() {
    ...
    var is_safari = navigator.userAgent.indexOf('applewebkit') > -1 && navigator.userAgent.indexOf('mobile') > -1 && navigator.userAgent.indexOf('safari') > -1;
...
function changeMap() {
    if (locations.c == undefined) return;
    mapid = locations.c.m;
    if (mapid == 0 && map != 0) {
        map = 0;
        radar.setMap(maps[0]);
...
function redraw() {
    ...
    if (!locations.p) return;
    var cfg = locations.c;
    fight = cfg.b;
    if (isInFinal) fight = 2;
    if (cfg.n != undefined) {
    ...
    if (cfg.p != undefined) {
        ...
        if (part_text == "") $('#id_aimbot_lbl').text("Aimbot");
        else $('#id_aimbot_lbl').text("Aimbot[" + part_text + "]");
    }
    if (cfg.s != undefined) {
        $('#id_buddyShare').prop({
            checked: cfg.s
...
function auto_zoom() {
    if (is_auto_zoom) {
        ...
        if (Math.abs(sc) + 0.3 > radar.scaleFactor && radar.scaleFactor > Math.abs(sc) + 0.3) return;
        if (sc > 0) cus_zoom_out(sc);
        else cus_zoom_in(-sc);
        lastScale = sc;
    } else {
...
function drawPlayers() {
    ...
    for (var i = players.length - 1; i >= 0; i--) {
        ...
        if ((players[0].z > 20000) && isInAir == false) isInAir = true;
        if (isInTeam) {
            var _isAlone = true;
            for (var j = players.length - 1; j >= 0; j--) {
                if (!teamids.indexOf(players[j].t) === -1 || (players[j].t == __t && j != i)) {
                    _isAlone = false;//false
                    teamids.unshift(__t);
                    break;
                }
            }
        ...
        if (_dis >= 100 && _dis <= 300) is_enamary_100_300 = true;
    ...
    for (var i = 0; i < players.length; i++) {
        ...
        if (dis > 1200) continue;
        var showPlayerNumber = true;
        var kill = (player.k || 0);
        if (kill > 100 || kill < 0) isShowKill = false;
        ...
        if (isPlayerSelf) {
            ...
            color = 'lime';//rend_sc ? '#ff8000' : 'lime';//#61c757
            if (drawLongLine == 1) {
                radar.longLine(x1, y1, _R, 200, 1, player.r, "limegreen");
            }
            ...
            if (angle1 != 0 && isAlive) {
                ...
                if (dis > 300) floorInfo = '';
                if ((radar.scaledFactor > 1.8 || isEnemaryIn10m_curr_loop) && floorInfo != '') {
                    width = 38;
                    leftOffset = -20;
                }
                var str_info = "dis: "+ dis  //+ floorInfo;
                if (isShowKill == true && kill >= 1) {
                    str_info += "  kill:" + kill;
                    width = 40;
                    leftOffset = -23;
                }
                y = y1;
                radar.text1(x1, y, _R, str_info, "white", leftOffset, 12, width, 12, 20, 9);
                if(player.n != "") {
                    //radar.text1(x1, y, _R, player.n, "white", -12, 12, 20, 12, -15, 6);
                }
                //radar.text1(x1, y, _R, player.n, 'white', -12, 12, 20, 12, -15, 6);
                ...
                if (wstr != "") {
                    ...
                    if (wstr == "Grenade") wstr = "À×";
                    if (wstr == "98K" || wstr == "M24" || wstr == "À×" || wstr == "AWM") {
                        wcolor = "red";
        ...
        if (isGroggy) {
            ...
            if (isAlone && !isPlayerSelf && !isOldBrother) color = "Tomato";
            if (isInVis && radar.isLookatU(x1, y1, x0, y0, player0.r)) color = "#641E16";
            if (isTeamMem) color = "lime";
            radar.lineWithAngle(player.x, player.y, _R, arrow_wid, arrow_hei, player.r, color);
            radar.dot(x1, y1, _R, 'Black', color, dot_r, radius);
            
            if (isShowKill && isOldBrother) {
                radar.dot(x1, y1, _R, 'Black', "#9933ff", dot_r, radius);
            }
            radar.pieChart(player.x, player.y, _R, ((100 - h1) / 100), 'gray', 5);
            if (showPlayerNumber && !isPlayerSelf) {
                if (t > 100) num_display_mod = 0;
                var fontColor = isAlone ? "lime" : "white";
                if (num_display_mod == 1) {
                    if (kill == 0) radar.text(x1, y1, "", fontColor, _R, _R);
                    else radar.text(x1, y1, kill, fontColor, _R, _R);
                } else {
                    if (isTeamMem) {
                        //selfTeamIndex++;
                    }
                   ...
                   if (t<100) {
                    ...
                    else radar.text(x1, y1, "AI", fontColor, _R, _R);
                   //radar.text(x1, y1, idx, fontColor, _R, _R);
        ...
        if (rend_sc && isPlayerSelf) {
            ...
            radar.text1(x1, y1, _R, "OB:" + sc,'red', -12, 12, 20, 12, 15, 8);
        }
    }
}
function drawItems(an) {
    ...
    for (var i = items.length - 1; i >= 0; i--) {
        ...
        if (item.n == "d") {
            ...
            if (item.n == "88") radar.weapons(item.x, item.y, _R, item.n, 45);
            else radar.weapons(item.x, item.y, _R, item.n, 25);
            var floorInfo = radar.getFloorNum(locations.p[0].z, item.z);
            //if (radar.scaledFactor > 1.8 && floorInfo != '') radar.text1(item.x, item.y, _R, radar.getFloorNum(locations.p[0].z, item.z), "white", -15, 12.8, 30, 12, 22, 8);
        }
    }
}
function drawV(v) {}
function drawVehicles(an) {
    ...
    for (var j = vehicles.length - 1; j >= 0; j--) {
        ...
        if (vehicle.v == "a") {
            ...
            if (vehicle.v == "117") radar.cars(vehicle.x, vehicle.y, _R, vehicle.v, 29, 20);
            else radar.cars(vehicle.x, vehicle.y, _R, vehicle.v, 25, 17);
            if (!isNearFinal) drawV(vehicle);
        }
    }
}
